...
const COMMAND_DETAILS = {
  ...
  DEFAULT: {
    title: "READY TO TYPE",
    desc: "Lengkapi bagian _____ pada editor untuk menyelesaikan misi pintu otomatis.",
...
const PintuOtomatisSimulation = () => {
  ...
  const generateEducationalFeedback = (typedLine: string, lineIdx: number) => {
    ...
    // Deteksi jenis kesalahan dan berikan petunjuk spesifik
    if (trimmed === "" || trimmed.includes("_____")) {
      errorMsg +=
        "Baris ini sepertinya masih kosong atau belum selesai kamu isi.\n\n";

      // Berikan hint berdasarkan baris yang diharapkan
      if (expectedLine.includes("input")) {
        errorMsg +=
          "Petunjuk: Coba gunakan perintah INPUT untuk membaca sensor.\n\n";
        errorMsg +=
          "INPUT itu seperti mengambil data dari sensor gerak untuk diperiksa sistemnya 📡";
      } else if (expectedLine.includes("print")) {
        if (expectedLine.includes("terbuka")) {
          errorMsg +=
            "Petunjuk: Coba gunakan perintah PRINT untuk membuka pintu.\n\n";
          errorMsg +=
            "PRINT itu seperti memberikan instruksi ke motor pintu untuk membuka 🚪";
        } else {
          errorMsg +=
            "Petunjuk: Coba gunakan perintah PRINT untuk menutup pintu.\n\n";
          errorMsg +=
            "PRINT itu seperti memberikan instruksi ke motor pintu untuk menutup 🚪";
        }
      } else {
        errorMsg +=
          "Petunjuk: Coba perhatikan template yang sudah disediakan di editor 👀";
      }
      return errorMsg;
    }

    // Cek apakah kata kunci ada tapi salah penempatan/penulisan
    if (firstWord && COMMAND_GLOSSARY[firstWord]) {
      errorMsg += `${COMMAND_GLOSSARY[firstWord]}\n\n`;
      errorMsg += "Coba perhatikan lagi penulisan lengkapnya ya! 😊";
      return errorMsg;
    }

    // Kata kunci tidak dikenali sama sekali
    if (firstWord) {
      errorMsg += `Perintah "${firstWord}" belum dikenali oleh sistem.\n\n`;
    }

    if (expectedLine.includes("input")) {
      errorMsg +=
        "Petunjuk: Coba gunakan perintah INPUT untuk membaca data dari sensor.\n\n";
      errorMsg +=
        "INPUT itu fungsinya untuk mengambil data dari perangkat luar,\n";
      errorMsg += "seperti sensor infrared yang mendeteksi kehadiran orang 📡";
    } else if (expectedLine.includes("print")) {
      errorMsg +=
        "Petunjuk: Coba gunakan perintah PRINT untuk mengirim perintah ke pintu.\n\n";
      errorMsg +=
        "PRINT itu fungsinya untuk memberikan instruksi ke perangkat keluaran,\n";
      errorMsg += "seperti motor pintu yang menggerakkan pintu otomatis 🚪";
    } else {
      errorMsg += "Petunjuk: Pastikan menggunakan perintah yang sesuai ya!\n\n";
      errorMsg += "Kalau bingung, lihat contoh di panel sebelah kiri 😊";
    }
  ...
  const executeStep = async (index: number): Promise<void> => {
    ...
    if (lineParsed.includes("start")) {
      ...
      setSimState((prev) => ({
        ...prev,
        isProcessing: true,
        feedback: "IF: Mengevaluasi data sensor...",
      }));
      ...
      setSimState((prev) => ({
        ...
        isProcessing: false,
        feedback: match
          ? "Hasil: BENAR! (Orang terdeteksi)"
          : "Hasil: SALAH. (Tidak ada orang)",
      }));
      ...
      if (lineParsed.includes("terbuka")) {
        ...
        setSimState((prev) => ({
          ...prev,
          feedback: "PRINT: Membuka pintu otomatis.",
        }));
      } else if (lineParsed.includes("tertutup")) {
        updateSimData({ doorOpen: false });
        setSimState((prev) => ({
          ...prev,
          feedback: "PRINT: Menahan pintu tetap tertutup.",
        }));
      }
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#f8fafc]">
          ...
          <div className="flex-1 flex gap-5 px-6 pb-6 overflow-hidden">
            ...
            <aside className="w-[380px] bg-[#020617] rounded-3xl flex flex-col shrink-0 overflow-hidden shadow-2xl border border-slate-800 relative">
              ...
              <div className="h-[420px] p-6 flex flex-col items-center justify-center relative bg-[radial-gradient(circle_at_center,_#0f172a_0%,_#020617_100%)] overflow-hidden">
                {/* Sensor Infrared UI */}
              ...
              >
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <span
                    className={`text-[10px] font-black uppercase tracking-widest ${errorLine !== -1 ? "text-rose-600" : "text-slate-500"}`}
                  >
                    Logic Log
                  </span>
                </div>
                <div
                  className={`text-[11px] font-medium whitespace-pre-wrap leading-relaxed overflow-y-auto scrollbar-thin ${errorLine !== -1 ? "text-rose-700" : "text-emerald-400 font-mono"}`}
                >
                  {simState.feedback}
                </div>
              </div>
