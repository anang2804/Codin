...
const SIMULASI_SLUG = "variabel-terpadu-lanjutan-smartclass";
...
function createChallenge(): ChallengeData {
  return {
    skor: randomInt(68, 92),
    bonus: Number(`${randomInt(1, 8)}.${randomInt(0, 9)}`),
    hadir: Math.random() > 0.2,
  };
}
...
> = {
  ...
  char: {
    title: "CHAR",
    desc: "Tipe data karakter tunggal",
    color: "bg-amber-50 border-amber-200",
  },
  "+": {
    title: "ARITMATIKA (+)",
    desc: "Menjumlahkan skor dengan bonus",
  ...
  ">=": {
    title: "PEMBANDING (>=)",
    desc: "Menguji batas nilai minimal",
    color: "bg-emerald-50 border-emerald-200",
  },
  "<=": {
    title: "PEMBANDING (<=)",
    desc: "Menguji nilai lebih kecil/sama",
    color: "bg-rose-50 border-rose-200",
  },
  "==": {
    title: "PEMBANDING (==)",
    desc: "Menguji nilai sama",
    color: "bg-fuchsia-50 border-fuchsia-200",
  },
  "&&": {
    title: "LOGIKA (&&)",
    desc: "Kondisi harus sama-sama true",
    color: "bg-teal-50 border-teal-200",
  },
  "||": {
    title: "LOGIKA (||)",
    desc: "Salah satu kondisi true",
    color: "bg-cyan-50 border-cyan-200",
  },
  default: {
    title: "LANJUTAN",
    desc: "Gabungkan variabel, aritmatika, pembanding, dan boolean logic.",
    color: "bg-slate-50 border-slate-200",
  },
};

export default function VariabelTerpaduLanjutanPage() {
  ...
  const nilaiAkhir = challenge.skor + challenge.bonus;
  const lulus = nilaiAkhir >= 75;
  const rekomendasi = lulus && challenge.hadir;
  const grade = lulus ? "B" : "C";

  const lineConfigs: LineConfig[] = [
    {
      before: "",
      after: ` skor = ${challenge.skor};`,
      expected: "int",
      choices: ["int", "char", "+", "=="],
    },
    {
      before: "float nilai_akhir = skor ",
      after: ` ${challenge.bonus.toFixed(1)};`,
      expected: "+",
      choices: ["+", "-", "*", "/"],
    },
    {
      before: "boolean lulus = nilai_akhir ",
      after: " 75;",
      expected: ">=",
      choices: [">=", "<=", "==", "+"],
    },
    {
      before: "boolean rekomendasi = lulus ",
      after: ` hadir; // hadir = ${challenge.hadir}`,
      expected: "&&",
      choices: ["&&", "||", "==", "*"],
    },
    {
      before: "",
      after: " grade = lulus ? 'B' : 'C';",
      expected: "char",
      choices: ["char", "int", "==", "-"],
    },
  ];
  ...
  const executeStep = (index: number) => {
    if (index >= lineConfigs.length) {
      ...
      setFeedback("Keren! Kamu menuntaskan level lanjutan.");
      return;
    }
    ...
    if (chosen !== expected) {
      ...
      setErrorLine(index);
      setFeedback(
        `Baris ${index + 1} belum tepat. Token yang benar: ${expected}.`,
      );
      return;
    }

    if (index === 0) {
      setSmartPreview((prev) => ({ ...prev, skor: challenge.skor }));
    }
    if (index === 1) {
    ...
    if (index === 2) {
      setSmartPreview((prev) => ({ ...prev, lulus }));
    }
    if (index === 3) {
      setSmartPreview((prev) => ({ ...prev, rekomendasi }));
    }
    if (index === 4) {
      setSmartPreview((prev) => ({ ...prev, grade }));
    }

    setFeedback(`Baris ${index + 1} benar.`);
    timerRef.current = setTimeout(() => executeStep(index + 1), 900);
  };
      ...
      <main className="flex flex-1 overflow-hidden">
        ...
        <div className="flex min-w-0 flex-1 flex-col bg-transparent">
              ...
              <div className="flex-1">
                <div className="mb-1 flex items-center gap-2">
                  ...
                  <h2 className="text-[15px] font-black uppercase tracking-tight text-foreground">
                    Keputusan Nilai Berbasis Logika
                  </h2>
                </div>
                <p className="max-w-4xl text-[11px] font-medium leading-relaxed text-muted-foreground">
                  Lengkapi token agar sistem smart class dapat menentukan
                  rekomendasi dan grade akhir.
                </p>
              </div>
            </div>
          </section>
              ...
              >
                <div className="rounded-2xl border border-emerald-200 bg-white px-4 py-3 shadow-sm">
                  ...
                  <p className="mt-1 text-[12px] font-medium leading-relaxed text-muted-foreground">
                    nilai_akhir = {nilaiAkhir.toFixed(1)}, lulus ={" "}
                    {String(lulus)}, rekomendasi = {String(rekomendasi)}, grade
                    = '{grade}'.
                  </p>
                </div>
              </motion.section>
          ...
          <div className="flex flex-1 gap-5 overflow-hidden px-6 pb-6">
                    ...
                    {lineConfigs.map((line, i) => {
                      ...
                      return (
                        ...
                        >
                          {isActive && (
                            <motion.div
                              layoutId="lineHighlightLanjutan"
            ...
            <aside className="relative flex w-[380px] shrink-0 flex-col overflow-hidden rounded-3xl border border-indigo-300/60 bg-gradient-to-b from-indigo-50 via-blue-50 to-cyan-100 shadow-2xl">
              <div className="flex items-center justify-between border-b border-indigo-300/60 bg-white/80 px-6 py-4">
                <h2 className="text-[10px] font-black uppercase tracking-widest text-indigo-800">
                  SMART CLASS BOARD
                </h2>
                <div
                  className={`h-2.5 w-2.5 rounded-full ${isRunning ? "animate-pulse bg-emerald-500" : errorLine !== -1 ? "bg-rose-500" : "bg-indigo-500"}`}
                />
              </div>

              <div className="flex flex-1 flex-col gap-4 p-5 text-slate-800">
                <div className="rounded-2xl border border-indigo-300 bg-white/90 p-4 shadow-sm">
                ...
                <div className="rounded-xl border border-indigo-300 bg-white/90 p-4 shadow-sm">
                  <p className="text-[10px] font-black uppercase tracking-widest text-indigo-700">
                    Keputusan Akhir
                  </p>
                  <p className="mt-2 text-[12px] font-semibold text-slate-700">
                    boolean lulus = nilai_akhir {">="} 75;
                  </p>
                  <p className="text-[12px] font-semibold text-slate-700">
                    boolean rekomendasi = lulus && hadir;
                  </p>
                  <p className="mt-2 text-lg font-black text-indigo-900">
                    grade: '{smartPreview.grade || "-"}'
                  </p>
                </div>
              </div>
            </aside>
