...
const SIMULASI_SLUG = "variabel-terpadu-menengah-kasir";

type CommandChoice = "int" | "+" | "-" | "*" | "/" | ">=" | "<=" | "==";
...
export default function VariabelTerpaduMenengahPage() {
  ...
  const executeStep = (index: number) => {
    if (index >= lineConfigs.length) {
      ...
      setFeedback("Mantap! Variabel, aritmatika, dan pembanding sudah benar.");
      return;
    }
    ...
    if (chosen !== expected) {
      setIsRunning(false);
      setErrorLine(index);
      setFeedback(`Baris ${index + 1} salah. Token yang benar: ${expected}.`);
      return;
    }
      ...
      <main className="flex flex-1 overflow-hidden">
        ...
        <div className="flex min-w-0 flex-1 flex-col bg-transparent">
              ...
              <div className="flex-1">
                <div className="mb-1 flex items-center gap-2">
                  ...
                  <h2 className="text-[15px] font-black uppercase tracking-tight text-foreground">
                    Variabel + Aritmatika Belanja
                  </h2>
                </div>
                <p className="max-w-4xl text-[11px] font-medium leading-relaxed text-muted-foreground">
                  Lengkapi token agar proses hitung total, rata-rata, dan
                  validasi nilai berjalan benar.
                </p>
              </div>
            </div>
          </section>
              ...
              >
                <div className="rounded-2xl border border-emerald-200 bg-white px-4 py-3 shadow-sm">
                  ...
                  <p className="mt-1 text-[12px] font-medium leading-relaxed text-muted-foreground">
                    total = {total}, rata = {rata.toFixed(1)}, lulus ={" "}
                    {String(lulus)}.
                  </p>
                </div>
              </motion.section>
          ...
          <div className="flex flex-1 gap-5 overflow-hidden px-6 pb-6">
            ...
            <aside className="relative flex w-[380px] shrink-0 flex-col overflow-hidden rounded-3xl border border-amber-300/60 bg-gradient-to-b from-amber-50 to-orange-100 shadow-2xl">
              <div className="flex items-center justify-between border-b border-amber-300/60 bg-white/80 px-6 py-4">
                <h2 className="text-[10px] font-black uppercase tracking-widest text-amber-800">
                  MESIN KASIR
                </h2>
                <div
                  className={`h-2.5 w-2.5 rounded-full ${isRunning ? "animate-pulse bg-emerald-500" : errorLine !== -1 ? "bg-rose-500" : "bg-amber-500"}`}
                />
              </div>

              <div className="flex flex-1 flex-col gap-4 p-5 text-amber-900">
                <div className="rounded-2xl border border-amber-300 bg-slate-900 px-4 py-3 shadow-inner">
                ...
                <div className="rounded-xl border border-amber-300 bg-white/90 p-4 shadow-sm">
                  <p className="text-[10px] font-black uppercase tracking-widest text-amber-700">
                    Struk Belanja
                  </p>
                  <div className="mt-2 space-y-1 font-mono text-[12px] text-slate-700">
                    <p>tugas .......... {kasirPreview.tugas ?? "-"}</p>
                ...
                <div className="rounded-xl border border-amber-300 bg-white/90 p-4 shadow-sm">
                  ...
                  <p className="mt-2 text-[11px] font-semibold text-slate-700">
                    Gunakan + dan / untuk hitung, lalu bandingkan dengan {">="}{" "}
                    untuk evaluasi.
                  </p>
                </div>
              </div>
