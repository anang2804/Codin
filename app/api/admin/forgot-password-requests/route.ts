...
export async function POST(req: Request) {
  try {
    ...
    if (action === "approve") {
      ...
      // If admin provided password, use it; otherwise generate one
      let passwordToSet = newPassword;
      if (!passwordToSet) {
        passwordToSet = randomBytes(9)
          .toString("base64")
          .replace(/\/+=|\/+|\+/g, "A")
          .slice(0, 12);
      }

      // Validate password length if provided by admin
      ...
      return NextResponse.json({
        ok: true,
        message: "Permintaan disetujui dan password telah diubah",
